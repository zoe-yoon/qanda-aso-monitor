...
echo "=== ASO Daily Collect: $TODAY $(date +%H:%M:%S) ==="
...
if node src/collect.js 2>&1; then
  ...
  if grep -q "세션이 만료" "$LOG_FILE" 2>/dev/null || grep -q "sign_in" "$LOG_FILE" 2>/dev/null; then
    ...
    notify "error" "수집 단계에서 실패 (exit: $EXIT_CODE)"
  fi
  exit 1
fi
